...
def _init_command_inner(argv: Sequence[str]) -> int:
    ...
    if target.exists() and not opts.force:
        raise RunSiteError(f"{target} already exists; pass --force to overwrite.")
    ...
    if detected.has_uv:
        ...
        sys.stdout.write('  .venv/ detected — `executable = "auto"` will pick its python\n')
    else:
        sys.stdout.write(
    ...
    sys.stdout.write(
        "\nNext: run `run-site doctor` to sanity-check, or `run-site run` to start the stack.\n"
    )
    return 0
...
def _render_toml(detected: DetectedDefaults, *, with_celery: bool) -> str:
    ...
    lines: list[str] = []
    lines.append(
        "# Generated by `run-site init`. Edit freely; `run-site doctor` validates the result."
    )
    lines.append("")
    ...
    if detected.has_uv:
        ...
        lines.append(
            '# Switch to `command = ["uv", "run", "--no-sync", "python"]` once uv is installed.'
        )
    ...
    lines.append("migrate_on_change = true")
